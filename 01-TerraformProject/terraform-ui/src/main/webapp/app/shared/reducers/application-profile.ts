import axios from 'axios';

import { SUCCESS } from 'app/shared/reducers/action-type.util';

import { fetch_managementinfo } from './authorized_account';

export const ACTION_TYPES = {
  GET_PROFILE: 'applicationProfile/GET_PROFILE'
};

// for profile = 'prod' or 'swagger'
const initialState = {
  ribbonEnv: '',
  inProduction: true,
  isSwaggerEnabled: false
};

export type ApplicationProfileState = Readonly<typeof initialState>;

export default (state: ApplicationProfileState = initialState, action): ApplicationProfileState => {
  console.log('application-profiles.ts ENTRY = ', { action, state });
  console.log('application-profiles.ts action.type=', action.type);
  switch (action.type) {
    case SUCCESS(ACTION_TYPES.GET_PROFILE):
      const { data } = action.payload;
      console.log('application-profiles.ts GET_PROFILE, data= ', data);
      console.log('application-profiles.ts GET_PROFILE RETURN= ', {
        ...state,
        ribbonEnv: data['display-ribbon-on-profiles'],
        inProduction: data.activeProfiles.includes('prod'),
        isSwaggerEnabled: data.activeProfiles.includes('swagger')
      });

      return {
        ...state,
        ribbonEnv: data['display-ribbon-on-profiles'],
        inProduction: data.activeProfiles.includes('prod'),
        isSwaggerEnabled: data.activeProfiles.includes('swagger')
      };
    default:
      return state;
  }
};

export const getProfile = () => ({
  type: ACTION_TYPES.GET_PROFILE,
  payload: fetch_managementinfo()
  // payload: axios.get('management/info')
});
